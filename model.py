...
def main(verbose=None, *args, xml=None, txt=None, dir=None, **kwargs):
    ...
    def get_file_data(f): return next(geo.txt_pipeline(os.path.join(dir, f + '*')))

    train = (dict(zip(x.values(), map(get_file_data, x))) for x in train_files)
    test = (dict(zip(x.values(), map(get_file_data, x))) for x in test_files)
    print ( ( (next(train)) ) )
